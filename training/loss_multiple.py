...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, D, DAux, diffaugment='',  augment_pipe=None, augment_pipe_cv=None,
        ...
        self.augment_pipe_cv = augment_pipe_cv
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain, loss_multiplier=1.):
        ...
        if do_Gmain:
            ...
            with torch.autograd.profiler.record_function('Gmain_backward'):
                if self.cv_type is not None:
                    (loss_Gmain+ total_loss).mean().mul(gain).backward()
                else:
        ...
        if do_Dmain:
            ...
            with torch.autograd.profiler.record_function('Dgen_backward'):
                if self.cv_type is not None:
                    (loss_Dgen+total_loss).mean().mul(gain).backward()
                else:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                if self.cv_type is not None:
                    ...
                    for i in range(len(self.cv_ensemble.models)):
                        if isinstance(logits_aux[i], list):
                            training_stats.report('Loss/signs/real'+ str(i), logits_aux[i][-1].sign())
                        else:
                            training_stats.report('Loss/signs/real'+ str(i), logits_aux[i].sign())
                else:
                ...
                if do_Dmain:
                    if self.cv_type is not None:
                        aux_loss = self.aux_loss.loss(logits_aux, for_real = True)
                        for i, each in enumerate(aux_loss):
                            training_stats.report('Loss/D/aux_loss_real' + str(i), each)
                            total_loss += each
            ...
            with torch.autograd.profiler.record_function(name + '_backward'):
                if self.cv_type is not None:
                    ...
                    if isinstance(logits_aux, list):
                        for each in logits_aux:
                            if isinstance(each, list):
                                loss_aux_mask += sum([x.sum() for x in each]) * 0
                            else:
                                loss_aux_mask += each.sum() * 0
                    else:
                        loss_aux_mask = logits_aux.sum() * 0
                        
                    (real_logits.sum() * 0 + loss_aux_mask * 0 + loss_Dreal + loss_Dr1 + total_loss).mean().mul(gain).backward()
                else:
