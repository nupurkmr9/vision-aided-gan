...
def training_loop(
    ...
    if augment_kwargs_cv is not None and (augment_p > 0 or ada_target_cv is not None):
        augment_pipe_cv = dnnlib.util.construct_class_by_name(**augment_kwargs_cv, num_p=len(cv_ensemble.models)).train().requires_grad_(False).to(device)
    ...
    loss = dnnlib.util.construct_class_by_name(device=device, **ddp_modules, **loss_kwargs , cv_type=cv_type, cv_loss=cv_loss)
    ...
    while True:
        ...
        if (ada_stats_cv is not None) and (batch_idx % ada_interval == 0):
            if (augment_pipe_cv is not None) and (not warmup or (warmup and cur_nimg>5e5)):
