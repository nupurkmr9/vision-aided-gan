...
def launch(cmd, kimgs_list, num_models=3):
    cv_models_list = [
    ...
    if acc > 0.9 or 'diffaugment' in augcv:
        cv_loss += '_s'
    run_cmd = cmd + ' --cv-loss ' + cv_loss + ' --cv '+ cv + ' --kimg ' + kimgs_list.split(',')[0]
    ...
    for num in range(num_models-1):
        ...
        run_cmd = cmd + ' --cv-loss ' + cv_loss + ' --cv '+ cv + ' --exact-resume 2 ' + ' --resume ' + network_pkl + ' --kimg ' + kimgs_list.split(',')[num+1]
...
if __name__ == "__main__":
    args = parse_args()
    model_name = launch(cmd= args.cmd, kimgs_list=args.kimgs_list)