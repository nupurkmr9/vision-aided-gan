...
from vision_module.DiffAugment_pytorch import DiffAugment
...
class CVBackbone(torch.nn.Module):
    ...
    def __init__(self, cv_type, output_type, diffaug=False, device='cpu'):
        ...
        self.class_name_dict = {
                'seg_ade': 'vision_module.swintaskspecific.Swin',
                'det_coco': 'vision_module.swintaskspecific.Swin',
                'clip': 'vision_module.cvmodel.CLIP',
                'dino': 'vision_module.cvmodel.DINO',
                'vgg': 'vision_module.cvmodel.Vgg',
                'swin': 'vision_module.cvmodel.Swin',
                'face_seg': 'vision_module.face_parsing.Parsing',
                'face_normals': 'vision_module.face_normals.Normals'
            }
    ...
    def __call__(self, images):
        ...
        if isinstance(images, list):
            for i, each in enumerate(self.models):
                image_features.append(each(DiffAugment(images[i], policy=self.policy)))
        else:
            for i, each in enumerate(self.models):
