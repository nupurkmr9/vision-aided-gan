...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, D, cvD=None, diffaugment=None,  augment_pipe=None, augment_pipe_cv=None,
                 cv_loss=None, num_cv_models=0, style_mixing_prob=0.9, r1_gamma=10, pl_batch_shrink=2,
        ...
        self.cvD = cvD
        self.num_cv_models = num_cv_models
        if cv_loss is not None:
            self.vision_aided_loss = dnnlib.util.construct_class_by_name(class_name='vision_module.cv_losses.losses_list', loss_type=cv_loss).to(device)
        ...
        if self.cvD is not None:
            if isinstance(self.augment_pipe_cv, str) and 'diffaug' in self.augment_pipe_cv:
                img_cv = [DiffAugment(img, policy=self.augment_pipe_cv.split('diffaug-')[1])
                          for _ in range(self.num_cv_models)]
                ...
                img_cv = [DiffAugment(self.augment_pipe_cv(img, prob=self.augment_pipe_cv.p[i]), policy='cutout')
                          for i in range(self.num_cv_models)]
            else:
                img_cv = [img for i in range(self.num_cv_models)]

            with misc.ddp_sync(self.D, sync):
                logits = self.D(img1, c)
            with misc.ddp_sync(self.cvD, sync):
                logits_cv = self.cvD(img_cv, c, detach)
