...
def training_loop(
    ...
    cur_nimg                = 0,        # nimg from which resuming if resume
    desc                    = '',       # name for the training run
    ...
    augment_kwargs_cv       = None,     # Options for augmentation for the vision model based discriminator. None = disable.  
    ada_target_cv           = None,
    ...
    if augment_kwargs_cv is not None and (augment_p > 0 or ada_target_cv is not None):
        augment_pipe_cv = dnnlib.util.construct_class_by_name(**augment_kwargs_cv, num_p=num_cv_models).train().requires_grad_(False).to(device)
        augment_pipe_cv.p.copy_(torch.as_tensor(augment_p))
        if ada_target_cv is not None:
            ada_stats_cv = []
            for i in range(num_cv_models):
                ada_stats_cv.append(training_stats.Collector(regex='Loss/signs/real' + str(i)))
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        if cvD is not None and 'cvD' in resume_data and resume_data['cvD'] is not None:
    ...
    if (resume_pkl is not None and exact_resume > 0) and (augment_pipe is not None):
        with dnnlib.util.open_url(resume_pkl) as f:
            resume_data = legacy.load_network_pkl(f)
        augment_pipe.p.copy_(resume_data['augment_pipe'].p)


    if (resume_pkl is not None and exact_resume > 0) and augment_pipe_cv is not None:
        with dnnlib.util.open_url(resume_pkl) as f:
            resume_data = legacy.load_network_pkl(f)
        if 'augment_pipe_cv' in resume_data and resume_data['augment_pipe_cv'] is not None:
            for i in range(len(resume_data['augment_pipe_cv'].p)):
                augment_pipe_cv.p[i].copy_(resume_data['augment_pipe_cv'].p[i])

    if (resume_pkl is not None and exact_resume > 0) and (augment_pipe is None):
        with dnnlib.util.open_url(resume_pkl) as f:
            resume_data = legacy.load_network_pkl(f)
    ...
    if rank == 0:
        ...
        if cvD is not None:
            misc.print_module_summary(cvD, [img, c] )
    ...
    for name, module in [('G_mapping', G.mapping), ('G_synthesis', G.synthesis), ('D', D), ('cvD', cvD), (None, G_ema), ('augment_pipe', augment_pipe),('augment_pipe_cv', augment_pipe_cv)]:
        if (num_gpus > 1) and (module is not None) and len(list(module.parameters())) != 0:
            if name == 'cvD':
                module.cv_ensemble.requires_grad_(False)
                module.decoder.requires_grad_(True)
            else:
    ...
    for name, module, opt_kwargs, reg_interval in phase_list:
        if reg_interval is None:
            ...
            if name == 'cvD' and cvD is not None:
                ...
                if num_cv_models >= 1 and exact_resume == 2:
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            if 'D' in phase.name and cvD is not None:
                phasecvD.opt.zero_grad(set_to_none=True)
                phasecvD.module.decoder.requires_grad_(True)
                phasecvD.module.cv_ensemble.requires_grad_(False)
        ...
        if (ada_stats_cv is not None) and (batch_idx % ada_interval == 0):
            if (augment_pipe_cv is not None) and (cur_nimg > warmup):
                for i, stat in enumerate(ada_stats_cv):
                    stat.update()
                    adjust = np.sign(stat['Loss/signs/real' + str(i)] - ada_target_cv) * (batch_size * ada_interval) / (ada_kimg * 1000)
                    augment_pipe_cv.p[i].copy_((augment_pipe_cv.p[i] + adjust).max(misc.constant(0, device=device)))
        ...
        if cvD is not None:
            for i in range(num_cv_models):
                fields += [f"augmentcv {training_stats.report0('Progress/augmentcv' + str(i), float(augment_pipe_cv.p[i].cpu()) if augment_pipe_cv is not None else 0):.3f}"]
        ...
        if ((network_snapshot_ticks is not None ) and (done or cur_tick % network_snapshot_ticks == 0) ):
            ...
            for name, module in [('G', G), ('D', D), ('G_ema', G_ema), ('cvD', cvD), ('augment_pipe', augment_pipe), ('augment_pipe_cv', augment_pipe_cv)]:
                ...
                if module is not None:
                    if num_gpus > 1:
                        misc.check_ddp_consistency(module, ignore_regex=r'.*\.*_avg')
        ...
        if (snapshot_data is not None) and (len(metrics) > 0):
            ...
            for metric in metrics:
                result_dict = metric_main.calc_metric(metric=metric, G=snapshot_data['G_ema'],
                        dataset_kwargs=training_set_kwargs, num_gpus=num_gpus, rank=rank, device=device, clean=clean)
        ...
        if stats_jsonl is not None:
            ...
            stats_jsonl.flush()
        if stats_tfevents is not None or wandb_log:
            global_step = int(cur_nimg / 1e3)
            walltime = timestamp - start_time
            if wandb_log:
                if rank ==0 :
                    wandb.log({name: value.mean for name, value in stats_dict.items()} , step=global_step)
                ...
                for name, value in stats_metrics_val.items():
                    stats_tfevents.add_scalar(f'MetricsVal/{name}', value, global_step=global_step, walltime=walltime)
