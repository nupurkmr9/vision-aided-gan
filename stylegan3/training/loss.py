...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G, D, cvD, augment_pipe=None, cv_loss=None, num_models=1, r1_gamma=10, style_mixing_prob=0, pl_weight=0, pl_batch_shrink=2, pl_decay=0.01, pl_no_weight_grad=False, blur_init_sigma=0, blur_fade_kimg=0):
        super().__init__()
        self.device             = device
        self.G                  = G
        self.D                  = D
    ...
    def run_D(self, img, c, blur_sigma=0, update_emas=False, detach=False):
        ...
        if self.cvD is not None:
            ...
            logits_cv = self.cvD(img, c, detach)
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, gain, cur_nimg, lambda_):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                if self.cvD is not None:
                    gen_logits, logits_cv = self.run_D(gen_img, gen_c, blur_sigma=blur_sigma)
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                if self.cvD is not None:
                    gen_logits, logits_cv = self.run_D(gen_img, gen_c, blur_sigma=blur_sigma, update_emas=True, detach=detach)
        ...
        if phase in ['Dmain', 'Dreg', 'Dboth']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                if self.cvD is not None:
                    real_logits, logits_cv = self.run_D(real_img_tmp, real_c, blur_sigma=blur_sigma, detach=detach)
