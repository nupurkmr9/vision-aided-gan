...
@click.option('--warmup', help='if training from scratch, train with standard adversarial loss for 500k images and then introduce vision-aided-loss', metavar='FLOAT', default=0.) 
@click.option('--cv-loss', default= 'sigmoid_loss', help='CV loss', type=str)
@click.option('--exact-resume', help='0: only resume model weights, 1: resume model weights ensuring exact match and augpipe, 2: resume model weights, optimizer and augpipe', type=bool, default=False, metavar='BOOL')
...
def main(**kwargs):
    ...
    #### CV supervision part ########
    if opts.cv is not None:
        cv = opts.cv
        
        output = cv.split('output-')[1]
        cv = cv.split('-output')[0].split('input-')[1]

        c.loss_kwargs.cv_loss = opts.cv_loss
        c.loss_kwargs.num_models = len(cv.split('+'))
        desc += f'-{cv:s}-cv_loss_{opts.cv_loss:s}'

        c.cvD_kwargs = dnnlib.EasyDict(class_name='vision_aided_loss.Discriminator', cv_type= cv, output_type=output, diffaug=True)
        c.cvD_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', betas=[0,0.99], eps=1e-8)
        c.cvD_opt_kwargs.lr = opts.dlr
