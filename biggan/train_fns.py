...
def Vision_aided_GAN_training_function(G, D, cvD, GD, z_, y_, ema, state_dict, config):
    def train(x, y, itr):
        G.optim.zero_grad(set_to_none=True)
        D.optim.zero_grad(set_to_none=True)
        cvD.optim.zero_grad(set_to_none=True)
        # How many chunks to split x and y into?
        x = torch.split(x, config['batch_size'])
        y = torch.split(y, config['batch_size'])
        counter = 0

        if itr > config['warmup']:
            lambda_ = 1.
        else:
            lambda_ = 0.
        ...
        for step_index in range(config['num_D_steps']):
            ...
            for accumulation_index in range(config['num_D_accumulations']):
                ...
                D_loss_CR = 0
                if config['CR'] > 0:
                    [D_fake, D_real, D_real_aug], [D_fake2, D_real2] = D_scores
                    D_loss_CR = torch.mean(
                        (D_real_aug - D_real) ** 2) * config['CR']
                else:
                    [D_fake, D_real],  [D_fake2, D_real2] = D_scores
                ...
                D_loss_real2, D_loss_fake2 = losses.cvdiscriminator_loss(D_fake2, D_real2)
                D_loss2 = (D_loss_real2 + D_loss_fake2) / float(config['num_D_accumulations'])

                D_loss = D_loss_real + D_loss_fake + D_loss_CR
                D_loss = D_loss / float(config['num_D_accumulations'])

                (D_loss+lambda_*D_loss2).backward()
        ...
        if config['toggle_grads']:
            ...
            cvD.decoder.requires_grad_(False)
            cvD.cv_ensemble.requires_grad_(False)
        ...
        if not config['fix_G']:
            ...
            for accumulation_index in range(config['num_G_accumulations']):
                z_.sample_()
                y_.sample_()
                D_fake, D_fake2 = GD(z_, y_, train_G=True, policy=config['DiffAugment'])
                G_loss = losses.generator_loss(
                    D_fake) / float(config['num_G_accumulations'])
                G_loss2 = losses.cvgenerator_loss(D_fake2) / float(config['num_G_accumulations'])
                (G_loss + lambda_*G_loss2).backward()
        ...
        out = {'G_loss': float(G_loss.item()) if not config['fix_G'] else 0,
               'G_loss2': float(G_loss2.item()) if not config['fix_G'] else 0,
               'D_loss_real': float(D_loss_real.item()),
               'D_loss_real2': float(D_loss_real2.item()),
               'D_loss_fake': float(D_loss_fake.item()),
               'D_loss_fake2': float(D_loss_fake2.item()),
               'D_acc0': (D_real > 0).float().mean().item(),
               }
        if config['CR'] > 0:
            out['D_loss_CR'] = float(D_loss_CR.item())
        for i in range(len(D_real2)):
            out['D_acc'+str(i+1)] = (D_real2[i] > 0).float().mean().item()
        # Return G's loss and the components of D's loss.
        return out
    return train
...
def save_and_sample(G, D, cvD, G_ema, z_, y_, fixed_z, fixed_y,
                    state_dict, config, experiment_name):
    utils.save_weights(G, D, cvD, state_dict, config['weights_root'],
...
def test(G, D, cvD, GD, G_ema, z_, y_, state_dict, config, sample, get_inception_metrics,
         experiment_name, test_log, acc_metrics, acc_itrs):
    D_acc_val = 0.
    D_acc_train = 0.
    if 'I128' not in config['dataset']:
