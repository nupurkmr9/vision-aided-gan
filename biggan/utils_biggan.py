...
def prepare_parser():
    ...
    parser.add_argument(
        ...
        help='dataset name for cleanfid evaluation, e.g. cifar10, cifar100')
    parser.add_argument(
        '--mirror_augment', action='store_true', default=False,
        help='Augment with random flips (default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--reset_optim', action='store_true', default=False,
        help='Reset optimizer? (default: %(default)s)')
    parser.add_argument(
        '--load_G_only', action='store_true', default=False,
        help='Load generator weights only? (default: %(default)s)')
    ...
    parser.add_argument(
        '--cv', type=str, default=None,
        help='CV module (default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--fix_G', action='store_true', default=False,
        help='Fix the weights in G? (default: %(default)s)')
    parser.add_argument(
        '--shared_dim', type=int, default=0,
        help='G''s shared embedding dimensionality; if 0, will be equal to dim_z. '
        '(default: %(default)s)')
    parser.add_argument(
        '--self_modulation', action='store_true', default=False,
        help='unconditonal training using self-modulation similar to U-NetGAN')
    parser.add_argument(
    ...
    parser.add_argument(
        '--CR_augment', type=str, default='flip,translation',
        help='CR Augmentation')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Suffix for which weights to load (e.g. best) '
        '(default: %(default)s)')
    parser.add_argument(
        '--resume_name', type=str, default=None,
...
dset_dict = {'I32': dset.ImageFolder, 'I64': dset.ImageFolder,
             ...
             'I128_hdf5': dset.ILSVRC_HDF5, 'I256_hdf5': dset.ILSVRC_HDF5, 'I128_0.25_hdf5': dset.ILSVRC_HDF5,
             'C10': dset.CIFAR10, 'C100': dset.CIFAR100}
imsize_dict = {'I32': 32, 'I32_hdf5': 32,
               'I64': 64, 'I64_hdf5': 64,
               'I128': 128, 'I128_hdf5': 128, 'I128_0.25_hdf5': 128,
               'I256': 256, 'I256_hdf5': 256,
               'C10': 32, 'C100': 32}
root_dict = {'I32': 'train', 'I32_hdf5': 'ILSVRC32.hdf5',
             'I64': 'train', 'I64_hdf5': 'ILSVRC64.hdf5',
             'I128': 'train', 'I128_hdf5': 'ILSVRC128.hdf5', 'I128_0.25_hdf5': 'ILSVRC0.25128.hdf5',
             'I256': 'train', 'I256_hdf5': 'ILSVRC256.hdf5',
             'C10': 'cifar', 'C100': 'cifar'}
nclass_dict = {'I32': 1000, 'I32_hdf5': 1000,
               'I64': 1000, 'I64_hdf5': 1000,
               'I128': 1000, 'I128_hdf5': 1000, 'I128_0.25_hdf5': 1000,
               'I256': 1000, 'I256_hdf5': 1000,
               'C10': 10, 'C100': 100}
# Number of classes to put per sample sheet
classes_per_sheet_dict = {'I32': 50, 'I32_hdf5': 50,
                          'I64': 50, 'I64_hdf5': 50,
                          'I128': 20, 'I128_hdf5': 20, 'I128_0.25_hdf5': 20,
...
def get_data_loaders(dataset, data_root=None, mirror_augment=False, batch_size=64,
                     num_workers=3, shuffle=True, load_in_mem=False, hdf5=False,
                     pin_memory=True, drop_last=True, start_itr=0,
                     num_epochs=500, use_multiepoch_sampler=False, num_samples=None, train=True, seed=0,
                     **kwargs):
    ...
    if 'hdf5' in dataset:
        ...
        if mirror_augment:
            ...
            if dataset in ['C10', 'C100']:
                ...
                # train_transform = []
                train_transform = [CenterCropLongEdge(), transforms.Resize(image_size)] # used in Imanget dataset
...
def save_weights(G, D, cvD, state_dict, weights_root, experiment_name,
    ...
    if cvD is not None:
        torch.save(cvD.state_dict(), 
              '%s/%s.pth' % (root, join_strings('_', ['cvD', name_suffix])))
        torch.save(cvD.optim.state_dict(),
              '%s/%s.pth' % (root, join_strings('_', ['cvD_optim', name_suffix])))
...
def load_weights(G, D, cvD, state_dict, weights_root, experiment_name,
    ...
    if G is not None:
        G.load_state_dict(
            torch.load('%s/%s.pth' %
                        (root, join_strings('_', ['G', name_suffix])), map_location='cpu'),
            strict=strict)
        if load_optim:
    ...
    if D is not None and not load_G_only:
        D.load_state_dict(
            torch.load('%s/%s.pth' %
                       (root, join_strings('_', ['D', name_suffix])), map_location='cpu'),
            strict=strict)
        if load_optim:
            D.optim.load_state_dict(
                torch.load('%s/%s.pth' % (root, join_strings('_', ['D_optim', name_suffix])), map_location='cpu'))
            
    if cvD is not None and not load_G_only:
        cvD.load_state_dict(
            torch.load('%s/%s.pth' %
                       (root, join_strings('_', ['cvD', name_suffix])), map_location='cpu'),
            strict=False)
    # Load state dict
    for item in state_dict:
        state_dict[item] = torch.load(
            '%s/%s.pth' % (root, join_strings('_', ['state_dict', name_suffix])), map_location='cpu')[item]
    if G_ema is not None:
        G_ema.load_state_dict(
            torch.load('%s/%s.pth' % (root, join_strings('_',
                                                         ['G_ema' if load_ema else 'G', name_suffix])), map_location='cpu'),
            strict=strict)
