...
def run(config):
    ...
    if config['cv'] is not None:
        ...
        cvD = cvDiscriminator(cv_type=cv, output_type=output, diffaug=True, create_optim=True,
                              num_classes=0 if config['self_modulation'] else config['n_classes'],
    ...
    print("best FID till now is:", state_dict['best_FID'])
        ...
        for i, (x, y) in enumerate(pbar):
            ...
            if not (state_dict['itr'] % config['save_every']):
                if config['G_eval_mode']:
                    ...
                    if config['ema']:
                        G_ema.eval()
                train_fns.save_and_sample(G, D, cvD, G_ema, z_, y_, fixed_z, fixed_y,
